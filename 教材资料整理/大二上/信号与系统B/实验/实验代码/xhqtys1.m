...
axes(h_axes);t=-5:0.001:5;y=cos(pi*t);plot(t,y,'b');title('原始信号');axis([-3 3 -1 1]);grid on;
axes(h_axes1);t=-5:0.001:5;y=cos(-pi*t);plot(t,y,'b');title('反折信号');axis([-3 3 -1 1]);grid on;
axes(h_axes2);t=-5:0.001:5;y=-cos(pi*t);plot(t,y,'b');title('倒相信号');axis([-3 3 -1 1]);grid on;
axes(h_axes3);w1=1;t=-5:0.001:5;y=cos(w1*pi*t);plot(t,y,'b');title('尺度变换');axis([-3 3 -1 1]);grid on;
axes(h_axes4);t1=0;t=-5:0.001:5;y=cos(pi*(t-t1));plot(t,y,'b');title('信号延迟');axis([-3 3 -1 1]);grid on;
...
't1=get(gcbo,''value'');' , ... % 获得滑动键状态值 
'other1(htext,str1,t1,htext1,str2,w1,h_axes3,h_axes4)' ]); % 被回调的函数文件 
str2= '尺度变换 =' ;
