...
axes(h_axes);t=-5:0.001:5;y=square(pi*t);plot(t,y,'b');title('原始信号');axis([-3 3 -1.5 1.5]);grid on;
axes(h_axes1);t=-5:0.001:5;y=square(-pi*t);plot(t,y,'b');title('反折信号');axis([-3 3 -1.5 1.5]);grid on;
axes(h_axes2);t=-5:0.001:5;y=-square(pi*t);plot(t,y,'b');title('倒相信号');axis([-3 3 -1.5 1.5]);grid on;
axes(h_axes3);w6=1;t=-5:0.001:5;y=square(pi*w6*t);plot(t,y,'b');title('尺度变换');axis([-3 3 -1.5 1.5]);grid on;
axes(h_axes4);t6=0;t=-5:0.001:5;y=square(pi*(t-t6));plot(t,y,'b');title('信号延迟');axis([-3 3 -1.5 1.5]);grid on;
...
't6=get(gcbo,''value'');' , ... % 获得滑动键状态值 
'other6(htext,str1,t6,htext1,str2,w6,h_axes3,h_axes4)' ]); % 被回调的函数文件 
str2= '尺度变换 =' ;
