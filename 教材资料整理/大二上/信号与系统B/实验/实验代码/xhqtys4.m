...
axes(h_axes);t=-5:0.001:5;y=exp(2*t);plot(t,y,'b');title('原始信号');axis([-3 3 0 10]);grid on;
axes(h_axes1);t=-5:0.001:5;y=exp(-2*t);plot(t,y,'b');title('反折信号');axis([-3 3 0 10]);grid on;
axes(h_axes2);t=-5:0.001:5;y=-exp(2*t);plot(t,y,'b');title('倒相信号');axis([-3 3 -10 0]);grid on;
axes(h_axes3);w4=1;t=-5:0.001:5;y=exp(w4*2*t);plot(t,y,'b');title('尺度变换');axis([-3 3 0 10]);grid on;
axes(h_axes4);t4=0;t=-5:0.001:5;y=exp(2*(t-t4));plot(t,y,'b');title('信号延迟');axis([-3 3 0 10]);grid on;
...
't4=get(gcbo,''value'');' , ... % 获得滑动键状态值 
'other4(htext,str1,t4,htext1,str2,w4,h_axes3,h_axes4)' ]); % 被回调的函数文件 
str2= '尺度变换 =' ;
