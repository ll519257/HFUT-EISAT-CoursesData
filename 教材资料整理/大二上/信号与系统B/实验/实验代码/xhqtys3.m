...
axes(h_axes);t=-5:0.001:5;t1=6;y=exp(-2*t).*((t>0)-(t>t1));plot(t,y,'b');title('原始信号');axis([-3 3 0 1]);grid on;
axes(h_axes1);t=-5:0.001:5;y=exp(2*t).*((t>-t1)-(t>0));plot(t,y,'b');title('反折信号');axis([-3 3 0 1]);grid on;
axes(h_axes2);t=-5:0.001:5;y=-exp(-2*t).*((t>0)-(t>t1));plot(t,y,'b');title('倒相信号');axis([-3 3 -1 0]);grid on;
axes(h_axes3);w3=1;t=-5:0.001:5;y=exp(-w3*2*t).*((t>0)-(t>t1));plot(t,y,'b');title('尺度变换');axis([-3 3 0 1]);grid on;
axes(h_axes4);t3=0;t=-5:0.001:5;y=exp(-2*(t-t3)).*((t>0)-(t>t1));plot(t,y,'b');title('信号延迟');axis([-3 3 0 1]);grid on;
...
't3=get(gcbo,''value'');' , ... % 获得滑动键状态值 
'other3(htext,str1,t3,htext1,str2,w3,h_axes3,h_axes4)' ]); % 被回调的函数文件 
str2= '尺度变换 =' ;
